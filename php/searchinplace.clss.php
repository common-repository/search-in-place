	...
	*/
	public function init() {
		...
		$this->javascriptVariables = array(
			'screen_reader_alert' => esc_html__( 'Search results pop-up.', 'search-in-place' ),
			'screen_reader_alert_instructions' => esc_html__( 'Press the Tab key to navigate through the search results.', 'search-in-place' ),
			'screen_reader_alert_result_single' => esc_html__( 'result', 'search-in-place' ),
			'screen_reader_alert_result_multiple' => esc_html__( 'results', 'search-in-place' ),
	...
	*/
	public function get_search_form( $atts = array() ) {
		...
		$_check_atts = function ( $atts, $att_name) {
			return (
				! empty( $atts[ $att_name ] ) &&
				is_numeric( $atts[ $att_name ] )
			) ? intval( $atts[ $att_name ] ) : 0;
		};

		// To avoid conficts with third-party plugins, it disables filters and actions
		$hooks_list = [ 'pre_get_search_form', 'search_form_args', 'search_form_format', 'get_search_form' ];
		$bk_hooks = [];
		foreach( $hooks_list as $hook_name ) {
			if ( ! empty( $wp_filter[ $hook_name ] ) ) {
				$bk_hooks[ $hook_name ] = $wp_filter[ $hook_name ];
			}
		}
		...
		$args = shortcode_atts(
			array( 'in_current_page' => 0, 'autocomplete' => 0 ),
			$atts
		);

		$in_current_page = $_check_atts( $args, 'in_current_page' );
		$autocomplete = $_check_atts( $args, 'autocomplete' );

		$search = str_replace(
			'name="s"',
			'name="s" data-search-in-place="1"' . ( $in_current_page ? ' data-search-in-page="1"' : '' ) . ( $autocomplete ? ' data-autocomplete="1"' : '' ),
			$search
		);

		if ( ! $in_current_page && $_check_atts( $atts, 'disable_enter_key' ) ) {
			$search = str_replace( 'name="s"', 'name="s" data-disable-enter-key="1"', $search );
		}

		if ( $in_current_page && $_check_atts( $atts, 'exclude_hidden_terms' ) ) {
		...
		if ( $in_current_page && $_check_atts( $atts, 'no_popup' ) ) {
	...
	*/
	public function modifySearchQuery( $query, $query_obj ) {
		...
		if ( ( ! is_admin() && $query_obj->is_main_query() && is_search() && isset( $_GET['s'] ) ) || ! empty( $cp_search_in_place ) ) {
			...
			$connection_operator = ( ( empty( $connection_operator ) || 'or' == strtolower( $connection_operator ) ) ? 'OR' : 'AND' );
	...
	public function populate() {
		...
		if ( 'exact' == strtolower( get_option( 'search_in_place_connection_operator', 'or' ) ) ) {
		...
		foreach ( $posts as $result ) {
			...
			if ( get_option( 'search_in_place_display_summary', 1 ) ) {
				...
				if ( ! empty( $result->post_excerpt ) ) {
					...
					$excerpt    = apply_filters( 'get_the_excerpt', $excerpt, $result );
	...
	*/
	...
	public function printAdminPage() {
		...
		if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'search-in-place-settings' ) && ! isset( $_POST['search_in_place_delete_settings'] ) ) {
			...
			$search_in_place_connection_operator = ( ! empty( $_POST['connection_operator'] ) && in_array( $_POST['connection_operator'], array( 'and', 'or', 'exact' ) ) ) ? sanitize_text_field( wp_unslash( $_POST['connection_operator'] ) ) : 'or';
		...
		echo '
			<div class="wrap">
				<style>
				.search-in-place-postbox .hndle{border-bottom:1px solid #DADADA;}
				.search-in-place-postbox .hndle span{padding: 5px 20px 10px 20px; display: inline-block;}
				.search-in-place-postbox h3{border-bottom: 1px dashed #ccc;}
				.post-type-container,.taxonomy-container{display:flex; flex-direction:row;}
				.post-type-container input[type="text"],.taxonomy-container input[type="text"]{margin-right: 10px;margin-left:0;flex-grow:1;}
				.button-primary,
				.button-secondary{min-height:45px!important;}
				...
				textarea:focus,select:focus,input[type="text"]:focus,input[type="number"]:focus{border-color:#2271b1!important;box-shadow:0 0 8px 0 rgb(205 208 210 / 30%)!important;outline:none!important;}
				textarea:hover,select:hover,input[type="text"]:hover,input[type="number"]:hover{border-color:#999!important;}
				textarea::placeholder,input[type="text"]::placeholder,input[type="number"]::placeholder{color:#999!important;font-style:italic!important;}
				</style>
				<form method="post" action="' . esc_attr( sanitize_text_field( isset( $_SERVER['REQUEST_URI'] ) ? wp_unslash( $_SERVER['REQUEST_URI'] ) : '' ) ) . '">' . wp_nonce_field( 'search-in-place-settings', '_wpnonce' ) . '
					...
					<div class="postbox search-in-place-postbox"><!-- General Settings -->
						<h3 class="hndle"><span>' . esc_html__( 'General Settings', 'search-in-place' ) . '</span></h3>
						<div class="inside">
					...
					<div class="postbox search-in-place-postbox"><!-- Elements to Display Postbox -->
						<h3 class="hndle"><span>' . esc_html__( 'Elements to display', 'search-in-place' ) . '</span></h3>
						<div class="inside">
							<table class="form-table">
								<tbody>
									<tr valign="top">
								...
								<tbody>
									...
									<tr valign="top">
										...
										<td>
											...
											<div id="label_background_start_color_picker"></div><br><br>
											Gradient end  color:
					...
					</div><!-- End Search in Page Design Postbox -->
					';
		...
		jQuery(function(){
			...
			controls.forEach(function(control) {
			  ...
			  control.parentNode.insertBefore(toggleSwitch, control);
